...
void CFileView::SaveUITree(HTREEITEM hItem, TiXmlElement* pParentNode)
{
	...
	while(hChild != NULL)
	{
		DWORD_PTR dwInfo = m_wndFileView.GetItemData(hChild);
...
void CFileView::OnFileNew()
{
	...
	DWORD_PTR dwInfo = m_wndFileView.GetItemData(hSelectedItem);
...
BOOL CFileView::RenameFile(CString strNewName, HTREEITEM hItem)
{
	...
	DWORD_PTR dwInfo = m_wndFileView.GetItemData(hItem);
	if(dwInfo == INFO_FILE)
	{
