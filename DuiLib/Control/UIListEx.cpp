...
namespace DuiLib {
	...
	void CListExUI::Notify(TNotifyUI& msg)
	{
		...
		// 全选框
		if(_tcsicmp(msg.sType.c_str(), _T("listheaditemchecked")) == 0)
		{
		...
		else if (_tcsicmp(msg.sType.c_str(), DUI_MSGTYPE_LISTITEMCHECKED) == 0)
		{
			for (int i = 0; i < GetCount(); ++i) {
				CControlUI* p = GetItemAt(i);
				CListTextExtElementUI* pLItem = static_cast<CListTextExtElementUI*>(p->GetInterface(_T("ListTextExElement")));
				...
				{
					OnListItemChecked(LOWORD(msg.wParam), HIWORD(msg.wParam), static_cast<BOOL>(msg.lParam != 0));
					break;
				}
			}
		}
		...
		if (_tcsicmp(strName.c_str(), _T("ListEx_Edit")) == 0 && m_pEditUI && m_nRow >= 0 && m_nColum >= 0)
		{
			...
			{
				tstring sText = m_pEditUI->GetText();
	...
	void CListExUI::OnListItemClicked(int nIndex, int nColum, RECT* lpRCColum, LPCTSTR lpstrText)
	{
		...
		else
		{
			if (CheckColumEditable(nColum) && GetEditUI())
			{
				...
				SIZE cxyFixed = { lpRCColum->left, lpRCColum->top };
				m_pEditUI->SetFixedXY(cxyFixed);
				// 设置编辑文本
	...
	{
		if( m_cxyFixed.cy == 0 ) {
			SIZE cxy = { m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 14 };
			return cxy;
		}
	...
	void CListContainerHeaderItemUI::PaintStatusImage(HDC hDC)
	{
		...
		if( (m_uButtonState & UISTATE_PUSHED) != 0 ) {
			if( m_sPushedImage.empty() && !m_sNormalImage.empty() ) DrawImage(hDC, m_sNormalImage.c_str());
			if( !DrawImage(hDC, m_sPushedImage.c_str()) ) {}
		}
		else if( (m_uButtonState & UISTATE_HOT) != 0 ) {
			if( m_sHotImage.empty() && !m_sNormalImage.empty() ) DrawImage(hDC, m_sNormalImage.c_str());
			if( !DrawImage(hDC, m_sHotImage.c_str()) ) {}
		}
		else if( (m_uButtonState & UISTATE_FOCUSED) != 0 ) {
			if( m_sFocusedImage.empty() && !m_sNormalImage.empty() ) DrawImage(hDC, m_sNormalImage.c_str());
		...
		if( !m_sSepImage.empty() ) {
			...
			DuiStringSmallFormat(m_sSepImageModify, _T("dest='%d,%d,%d,%d'"), rcThumb.left, rcThumb.top, rcThumb.right, rcThumb.bottom);
			if( !DrawImage(hDC, m_sSepImage.c_str(), m_sSepImageModify.c_str()) ) {}
		}
		...
		if(m_bCheckBoxable)
		{
			...
			if( (m_uCheckBoxState & UISTATE_SELECTED) != 0 ) {
				if( !m_sCheckBoxSelectedImage.empty() ) {
					if( !DrawCheckBoxImage(hDC, m_sCheckBoxSelectedImage.c_str()) ) {}
			...
			if( (m_uCheckBoxState & UISTATE_DISABLED) != 0 ) {
				if( !m_sCheckBoxDisabledImage.empty() ) {
					if( !DrawCheckBoxImage(hDC, m_sCheckBoxDisabledImage.c_str()) ) {}
					else return;
				}
			}
			else if( (m_uCheckBoxState & UISTATE_PUSHED) != 0 ) {
				if( !m_sCheckBoxPushedImage.empty() ) {
					if( !DrawCheckBoxImage(hDC, m_sCheckBoxPushedImage.c_str()) ) {}
					else return;
				}
			}
			else if( (m_uCheckBoxState & UISTATE_HOT) != 0 ) {
				if( !m_sCheckBoxHotImage.empty() ) {
					if( !DrawCheckBoxImage(hDC, m_sCheckBoxHotImage.c_str()) ) {}
					else return;
				}
			}
			else if( (m_uCheckBoxState & UISTATE_FOCUSED) != 0 ) {
				if( !m_sCheckBoxFocusedImage.empty() ) {
					if( !DrawCheckBoxImage(hDC, m_sCheckBoxFocusedImage.c_str()) ) {}
					else return;
				}
			}

			if( !m_sCheckBoxNormalImage.empty() ) {
				if( !DrawCheckBoxImage(hDC, m_sCheckBoxNormalImage.c_str()) ) {}
				else return;
			}

Label_ForeImage:
			if( !m_sCheckBoxForeImage.empty() ) {
				if( !DrawCheckBoxImage(hDC, m_sCheckBoxForeImage.c_str()) ) {}
			}
		}
	}
	...
	void CListContainerHeaderItemUI::PaintText(HDC hDC)
	{
		...
		int nLinks = 0;
		if( m_bShowHtml )
			CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, sText.c_str(), m_dwTextColor, \
			NULL, NULL, nLinks, DT_SINGLELINE | m_uTextStyle);
		else
			CRenderEngine::DrawText(hDC, m_pManager, rcText, sText.c_str(), m_dwTextColor, \
	...
	{
		return m_sCheckBoxNormalImage.c_str();
	}
	...
	{
		return m_sCheckBoxHotImage.c_str();
	}
	...
	{
		return m_sCheckBoxPushedImage.c_str();
	}
	...
	{
		return m_sCheckBoxFocusedImage.c_str();
	}
	...
	{
		return m_sCheckBoxDisabledImage.c_str();
	}
	...
	{
		return m_sCheckBoxSelectedImage.c_str();
	}
	...
	{
		return m_sCheckBoxForeImage.c_str();
	}
	...
	CListTextExtElementUI::~CListTextExtElementUI()
	{
		...
		for( int it = 0; it < m_aTexts.GetSize(); it++ ) {
			pText = static_cast<tstring*>(m_aTexts[it]);
	...
	{
		tstring* pText = static_cast<tstring*>(m_aTexts.GetAt(iIndex));
	...
	void CListTextExtElementUI::SetText(int iIndex, LPCTSTR pstrText)
	{
		...
		tstring* pText = static_cast<tstring*>(m_aTexts[iIndex]);
		if( (pText == NULL && pstrText == NULL) || (pText && *pText == pstrText) ) return;

		if ( pText )
			*pText = DuiStringAssign(pstrText);
		else
	...
	tstring* CListTextExtElementUI::GetLinkContent(int iIndex)
	{
	...
	void CListTextExtElementUI::PaintStatusImage(HDC hDC)
	{
		...
		for( int i = 0; i < pInfo->nColumns; i++ )
		{
			if (pListCtrl->CheckColumCheckBoxable(i))
			{
				...
				if( (m_uCheckBoxState & UISTATE_SELECTED) != 0 ) {
					if( !m_sCheckBoxSelectedImage.empty() ) {
						if( !DrawCheckBoxImage(hDC, m_sCheckBoxSelectedImage.c_str(), NULL, rcCheckBox) ) {}
						else goto Label_ForeImage;
					}
				}
				...
				if( (m_uCheckBoxState & UISTATE_DISABLED) != 0 ) {
					if( !m_sCheckBoxDisabledImage.empty() ) {
						if( !DrawCheckBoxImage(hDC, m_sCheckBoxDisabledImage.c_str(), NULL, rcCheckBox) ) {}
						else return;
					}
				}
				else if( (m_uCheckBoxState & UISTATE_PUSHED) != 0 ) {
					if( !m_sCheckBoxPushedImage.empty() ) {
						if( !DrawCheckBoxImage(hDC, m_sCheckBoxPushedImage.c_str(), NULL, rcCheckBox) ) {}
						else return;
					}
				}
				else if( (m_uCheckBoxState & UISTATE_HOT) != 0 ) {
					if( !m_sCheckBoxHotImage.empty() ) {
						if( !DrawCheckBoxImage(hDC, m_sCheckBoxHotImage.c_str(), NULL, rcCheckBox) ) {}
						else return;
					}
				}
				else if( (m_uCheckBoxState & UISTATE_FOCUSED) != 0 ) {
					if( !m_sCheckBoxFocusedImage.empty() ) {
						if( !DrawCheckBoxImage(hDC, m_sCheckBoxFocusedImage.c_str(), NULL, rcCheckBox) ) {}
						else return;
					}
				}

				if( !m_sCheckBoxNormalImage.empty() ) {
					if( !DrawCheckBoxImage(hDC, m_sCheckBoxNormalImage.c_str(), NULL, rcCheckBox) ) {}
					else return;
				}

Label_ForeImage:
				if( !m_sCheckBoxForeImage.empty() ) {
					if( !DrawCheckBoxImage(hDC, m_sCheckBoxForeImage.c_str(), NULL, rcCheckBox) ) {}
				}
			}
		}
	}
	...
	{
		CListExUI* pListCtrl = static_cast<CListExUI*>(m_pOwner);
		return pListCtrl != NULL ? pListCtrl->CheckColumEditable(nColum) : FALSE;
	}
